...
def on_open(import_info):
    ...
    for modulename, module, exc_info in import_info:
        if module is None:
            ...
            _log.error("examples.callbacks.on_open: Error loading '%s' : %s" % (modulename, exc_value))
...
def on_reload(import_info):
    ...
    for modulename, module, exc_info in import_info:
        if module is None:
            exc_type, exc_value, exc_traceback = exc_info
            _log.error("examples.callbacks.on_reload: Error loading '%s' : %s" % (modulename, exc_value))
            errors += 1
...
def on_close():
    ...
    if it's needed.
    """
    _log.info("examples.callbacks.on_close: PyXLL is closing")

@xl_license_notifier
def license_notifier(name, expdate, days_left):
    """
    ...
    """
    if days_left >= 0:
        _log.info(("examples.callbacks.license_notifier:\n"
                   "    This copy of PyXLL is licensed to %s\n"
                   "    %d days left before the license expires (%s)") % (name, days_left, expdate))
