"""
PyXLL Examples: Qt ActiveX control
...
try:
    # Try PySide6 if PyQt6 wasn't found
...
if not qt_imported:
    try:
        # Try PyQt5 if neither Qt6 packages were found
...
if not qt_imported:
    # Finally try PySide2 if no other packages were found
...
class ExampleQtWiget(QWidget):
    ...
    can be embedded in Excel as an ActiveX control.
    """
    ...
    def __init__(self, parent=None):
        ...
        image_path = os.path.join(os.path.dirname(__file__), "..", "ribbon", "icons", "qt.png")
        image = QLabel()
        image.setPixmap(QPixmap(image_path))
        label = QLabel("Qt ActiveX Control")
...
def show_qt_activex():
    """Create a Qt Widget and embed it in Excel as an ActiveX control."""
