"""
Extreme value type I (maximum) / Gumbel distribution support for Drisk.
...
def extvalue_generator_single(rng: np.random.Generator, params: List[float]) -> float:
    a = float(params[0])
    b = float(params[1])
    _validate_params(a, b)
    return float(rng.gumbel(loc=a, scale=b))


def extvalue_generator_vectorized(
    ...
    if np.any(b <= 0):
        raise ValueError("Extvalue requires b > 0")

    # scipy.stats.gumbel_r 的 loc 和 scale 支持数组
    from scipy.stats import gumbel_r
    return gumbel_r.rvs(loc=a, scale=b, size=n_samples, random_state=rng)


def extvalue_generator(
    rng: np.random.Generator,
    params: List[float],
    n_samples: Optional[int] = None,
) -> Union[float, np.ndarray]:
    if n_samples is None:
        return extvalue_generator_single(rng, params)
    return extvalue_generator_vectorized(rng, params, n_samples)


def extvalue_pdf(x: float, a: float, b: float) -> float:
    _validate_params(a, b)
    return float(sps.gumbel_r.pdf(x, loc=a, scale=b))


def extvalue_cdf(x: float, a: float, b: float) -> float:
    _validate_params(a, b)
    return float(sps.gumbel_r.cdf(x, loc=a, scale=b))


def extvalue_ppf(q_prob: float, a: float, b: float) -> float:
    _validate_params(a, b)
    if q_prob <= 0.0:
        return float("-inf")
    if q_prob >= 1.0:
    ...
    return float(sps.gumbel_r.ppf(q, loc=a, scale=b))


def extvalue_raw_mean(a: float, b: float) -> float:
    _validate_params(a, b)
    return float(a + b * _EULER_GAMMA)


class ExtvalueDistribution(DistributionBase):
    """Extreme value type I (maximum) distribution with truncation support."""
    ...
    def __init__(self, params: List[float], markers: dict = None, func_name: str = None):
        ...
        self.a = a
        self.b = b
        self._dist = sps.gumbel_r(loc=a, scale=b)
    ...
    def _original_cdf(self, x: float) -> float:
        return extvalue_cdf(x, self.a, self.b)

    def _original_ppf(self, q_prob: float) -> float:
        return extvalue_ppf(q_prob, self.a, self.b)

    def _original_pdf(self, x: float) -> float:
        return extvalue_pdf(x, self.a, self.b)
