...
def _validate_params(alpha: float, beta: float) -> None:
    if float(alpha) <= 0.0:
        raise ValueError("Weibull requires alpha > 0")
    if float(beta) <= 0.0:
        raise ValueError("Weibull requires beta > 0")


def _create_dist(alpha: float, beta: float):
    _validate_params(alpha, beta)
    return scipy_weibull_min(c=float(alpha), loc=0.0, scale=float(beta))


def weibull_pdf(x: float, alpha: float, beta: float) -> float:
    x = float(x)
    if x < 0.0:
        return 0.0
    _validate_params(alpha, beta)
    alpha = float(alpha)
    beta = float(beta)
    return float((alpha / (beta ** alpha)) * (x ** (alpha - 1.0)) * math.exp(-((x / beta) ** alpha)))


def weibull_cdf(x: float, alpha: float, beta: float) -> float:
    x = float(x)
    if x <= 0.0:
        return 0.0
    _validate_params(alpha, beta)
    return float(1.0 - math.exp(-((x / float(beta)) ** float(alpha))))


def weibull_ppf(q_prob: float, alpha: float, beta: float) -> float:
    q_prob = float(q_prob)
    _validate_params(alpha, beta)
    if q_prob <= 0.0:
        return 0.0
    if q_prob >= 1.0:
        return float("inf")
    return float(float(beta) * ((-math.log(1.0 - q_prob)) ** (1.0 / float(alpha))))


def weibull_raw_mean(alpha: float, beta: float) -> float:
    _validate_params(alpha, beta)
    return float(float(beta) * gamma(1.0 + 1.0 / float(alpha)))


def weibull_generator_single(rng: np.random.Generator, params: List[float]) -> float:
    alpha = float(params[0])
    beta = float(params[1])
    _validate_params(alpha, beta)
    return float(beta * rng.weibull(alpha))
...
class WeibullDistribution(DistributionBase):
    """Weibull theoretical distribution with truncation support."""

    def __init__(self, params: List[float], markers: dict = None, func_name: str = None):
        markers = markers or {}
        alpha = float(params[0]) if len(params) >= 1 else 2.0
        ...
        g1 = float(gamma(1.0 + 1.0 / alpha))
        g2 = float(gamma(1.0 + 2.0 / alpha))
        g3 = float(gamma(1.0 + 3.0 / alpha))
        g4 = float(gamma(1.0 + 4.0 / alpha))

        self._raw_mean = float(beta * g1)
        self._raw_var = float((beta ** 2) * (g2 - g1 ** 2))

        denom = g2 - g1 ** 2
        if denom > _EPS:
            self._raw_skew = float((g3 - 3.0 * g2 * g1 + 2.0 * (g1 ** 3)) / (denom ** 1.5))
            self._raw_kurt = float((g4 - 4.0 * g3 * g1 + 6.0 * g2 * (g1 ** 2) - 3.0 * (g1 ** 4)) / (denom ** 2))
        else:
            self._raw_skew = 0.0
            self._raw_kurt = 3.0

        if alpha > 1.0:
            self._raw_mode = float(beta * ((1.0 - 1.0 / alpha) ** (1.0 / alpha)))
        else:
            self._raw_mode = 0.0
    ...
    def _original_pdf(self, x: float) -> float:
        return weibull_pdf(x, self.alpha, self.beta)

    def _original_cdf(self, x: float) -> float:
        return weibull_cdf(x, self.alpha, self.beta)

    def _original_ppf(self, q_prob: float) -> float:
        return weibull_ppf(q_prob, self.alpha, self.beta)
