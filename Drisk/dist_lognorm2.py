...
def _validate_params(mu: float, sigma: float) -> None:
    if sigma <= 0.0:
        raise ValueError("Lognorm2 requires sigma > 0")


def _create_dist(mu: float, sigma: float):
    _validate_params(mu, sigma)
    return scipy_lognorm(s=float(sigma), scale=math.exp(float(mu)), loc=0.0)


def lognorm2_pdf(x: float, mu: float, sigma: float) -> float:
    if float(x) <= 0.0:
        return 0.0
...
def lognorm2_cdf(x: float, mu: float, sigma: float) -> float:
    if float(x) <= 0.0:
        return 0.0
    dist = _create_dist(mu, sigma)
...
def lognorm2_ppf(q: float, mu: float, sigma: float) -> float:
    q = float(q)
    if q <= 0.0:
        return 0.0
    if q >= 1.0:
...
def lognorm2_raw_mean(mu: float, sigma: float) -> float:
    _validate_params(mu, sigma)
    return float(math.exp(mu + 0.5 * sigma * sigma))


def lognorm2_generator_single(rng: np.random.Generator, params: List[float]) -> float:
    mu = float(params[0])
    sigma = float(params[1])
    q = float(rng.uniform(_EPS, 1.0 - _EPS))
    return lognorm2_ppf(q, mu, sigma)
...
class Lognorm2Distribution(DistributionBase):
    """Lognorm2 theoretical distribution with truncation support."""

    def __init__(self, params: List[float], markers: dict = None, func_name: str = None):
        markers = markers or {}
        mu = float(params[0]) if len(params) >= 1 else 1.263
        sigma = float(params[1]) if len(params) >= 2 else 1.0
        ...
        self._raw_mean = float(math.exp(mu + 0.5 * sigma * sigma))
        self._raw_var = float(math.exp(2.0 * mu) * omega * (omega - 1.0))
        self._raw_skew = float((omega + 2.0) * math.sqrt(max(0.0, omega - 1.0)))
        self._raw_kurt = float(omega ** 4 + 2.0 * omega ** 3 + 3.0 * omega ** 2 - 3.0)
        self._raw_mode = float(math.exp(mu - sigma * sigma))
    ...
    def _original_pdf(self, x: float) -> float:
        return lognorm2_pdf(x, self.mu, self.sigma)

    def _original_cdf(self, x: float) -> float:
        return lognorm2_cdf(x, self.mu, self.sigma)

    def _original_ppf(self, q_prob: float) -> float:
        return lognorm2_ppf(q_prob, self.mu, self.sigma)
